/*
  # Fix supplier_confirm_deal_v4 to handle in_delivery status

  When a deal is already in_delivery or execution_in_progress and the supplier
  calls supplier_confirm_deal_v4 (e.g., due to a frontend timing issue), instead
  of returning an error, redirect to confirm delivery directly.
*/
...
-- If already in delivery, tell frontend to use confirm delivery instead
IF v_deal.status IN ('in_delivery', 'execution_in_progress') THEN
RETURN jsonb_build_object('success', true, 'next_step', 'confirm_delivery', 'message', 'التسليم جارٍ، أكّد الإتمام');
END IF;
