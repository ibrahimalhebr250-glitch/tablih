/*
  # Fix Visitor Ratings to Auto-Create Comments

  ## Changes
  This migration updates the `create_visitor_rating` function to automatically create
  a comment entry in the `ratings_comments` table when a visitor submits a rating with a comment.

  ## Updated Functions
  - `create_visitor_rating` - Now creates an entry in ratings_comments if comment is provided

  ## Important Notes
  1. Comments are automatically approved for now (can be changed later)
  2. This ensures visitor comments appear in the CommentsSection component
  3. The comment is stored in both user_ratings.comment (for backward compatibility) 
     and ratings_comments table (for display in UI)
*/

-- Update create_visitor_rating function to auto-create comment
