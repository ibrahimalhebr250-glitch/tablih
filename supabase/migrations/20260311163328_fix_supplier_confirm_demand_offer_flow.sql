/*
  # Fix supplier confirm flow for demand_offer deals

  ## Problem
  For demand_offer deals:
  - When buyer accepts an offer → deal jumps directly to `inventory_reserved`
  - Supplier sees the deal and tries to confirm it
  - `supplier_confirm_deal_v4` rejects because `inventory_reserved` is not in allowed statuses

  Additionally, some demand_offer deals land in `awaiting_buyer` and need the supplier
  to confirm them which then skips the buyer re-confirmation step.

  ## Fix
  1. Add `inventory_reserved` to `supplier_confirm_deal_v4` allowed statuses — 
     for this status, skip reservation (already done) and go directly to `in_delivery`
     by delegating to start_delivery logic.
  2. For `demand_offer` deals in `awaiting_buyer`, allow supplier confirm to skip
     buyer re-confirmation and move straight to `inventory_reserved`.
*/
...
BEGIN
  ...
  -- Accept inventory_reserved too — means buyer already confirmed, just start delivery
  ...
  -- If already at inventory_reserved, move straight to in_delivery
  IF v_deal.status = 'inventory_reserved' THEN
    ...
    RETURN jsonb_build_object('success', true, 'next_step', 'in_delivery');
  END IF;
