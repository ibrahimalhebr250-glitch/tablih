/*
  # إصلاح مرجع UUID في دالة admin_get_inventory_batches

  1. المشكلة
    - inventory_images.batch_id هو UUID يشير إلى inventory_batches.id
    - الدالة كانت تستخدم ib.batch_id (TEXT) بدلاً من ib.id (UUID)
    - يسبب خطأ عدم توافق الأنواع

  2. الحل
    - تصحيح الاستعلام الفرعي لاستخدام ib.id بدلاً من ib.batch_id

  3. التأثير
    - تمكين عرض المخزون في لوحة التحكم الإدارية
    - عرض عدد الصور بشكل صحيح
*/
...
BEGIN
  ...
  SELECT 
    ...
    ib.approval_status,
    ib.image_url,
    (SELECT COUNT(*) FROM inventory_images ii WHERE ii.batch_id = ib.id) as images_count,
    ib.created_at,
    ib.updated_at
