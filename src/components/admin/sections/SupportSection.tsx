...
import {
  ...
  Loader2, RefreshCw, Users, MessageSquare, Clock, CheckCheck,
  AlertCircle, ChevronLeft, Filter, User, Building2, Phone,
  TrendingDown, Star
} from 'lucide-react';
    ...
    <div className={`flex ${isAdmin ? 'justify-start' : 'justify-end'}`}>
      <div style={{ maxWidth: '78%' }}>
        ...
        <div className={`flex items-center gap-1 mt-1 ${isAdmin ? 'justify-start' : 'justify-end'}`}>
          ...
          {!isAdmin && (
            <CheckCheck className={`w-3 h-3 ${msg.is_read ? 'text-[#0369A1]' : 'text-[#b0c4d0]'}`} />
          )}
        </div>
      </div>
    </div>
...
function ChatPanel({
  ...
  return (
    <div className="flex flex-col h-full" dir="rtl">
      {/* Chat Header */}
      <div
        className="flex items-center gap-3 px-4 py-3.5 flex-shrink-0 border-b border-gray-100"
      ...
      >
        ...
        <div
          className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 text-white font-bold"
          style={{ background: conv.user_type === 'supplier' ? '#059669' : '#0369A1' }}
        >
          {conv.user_name.charAt(0)}
        </div>
      ...
      <div className="flex items-end gap-2 px-4 py-3 flex-shrink-0 bg-white border-t border-gray-100" dir="rtl">
        ...
        <button
          ...
          className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0 hover:bg-gray-200 transition-colors disabled:opacity-50"
        >
        ...
        <button
          ...
          style={{ background: text.trim() ? 'linear-gradient(135deg, #0369A1, #0284C7)' : '#E5E7EB' }}
        >
          {sending ? (
...
export default function SupportSection({ adminEmail }: Props) {
  ...
  return (
    <div className="flex flex-col h-full" dir="rtl">
      {/* Header + Stats */}
      <div className="flex-shrink-0 p-4 lg:p-5 space-y-4 border-b border-gray-100 bg-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center shadow-md" style={{ background: 'linear-gradient(135deg, #0369A1, #0284C7)' }}>
              <Headphones className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="text-lg lg:text-xl font-black text-[#0f2535]">مركز دعم العملاء</h1>
              <p className="text-xs text-[#7a9aab]">رد مباشر على استفسارات العملاء من المنصة</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {totalUnread > 0 && (
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <StatCard icon={Users} label="محادثات" value={stats.total_conversations} color="#0369A1" />
            <StatCard icon={AlertCircle} label="غير مقروء" value={stats.unread_messages} color="#DC2626" sub={stats.unread_messages > 0 ? 'يحتاج رد' : ''} />
            <StatCard icon={MessageCircle} label="بدون رد" value={stats.unanswered_conversations} color="#D97706" />
            <StatCard
              icon={Clock}
              label="متوسط الرد"
              value={stats.avg_response_time_hours !== null ? `${stats.avg_response_time_hours} س` : '-'}
              color="#059669"
              sub="متوسط وقت الرد"
            />
          </div>
              ...
              {(['all', 'unread', 'unanswered'] as const).map(f => (
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all ${
                    filter === f ? 'bg-[#0369A1] text-white' : 'bg-gray-100 text-[#7a9aab] hover:bg-gray-200'
                  }`}
                >
            ...
            {conversations.length === 0 ? (
              ...
              conversations.map(conv => (
                <ConversationItem
                  key={conv.user_phone}
                  conv={conv}
                  isActive={conv.user_phone === activePhone}
                  onClick={() => handleSelect(conv.user_phone)}
                />
              ))
            )}
          </div>
        ...
        <div className={`flex-1 overflow-hidden ${!showChat || !activeConv ? 'hidden lg:flex lg:items-center lg:justify-center' : 'flex flex-col'}`}>
          {!activeConv ? (
            <div className="text-center px-6">
              ...
              <p className="text-lg font-black text-[#0f2535] mb-2">مركز دعم العملاء</p>
