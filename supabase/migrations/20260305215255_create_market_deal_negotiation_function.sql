/*
  # Create Market Deal Negotiation Function
  
  1. New Function
    - `create_deal_from_market_negotiation` - Creates a deal from market negotiation
    - Validates inventory batch availability
    - Creates deal with `pending_supplier` status
    - Reserves inventory quantity
    - Returns deal details including ID and reference
  
  2. Security
    - SECURITY DEFINER to allow users to create deals
    - Validates batch availability and status
...
BEGIN
  ...
  -- Get platform fee (default 1 SAR per pallet)
  SELECT COALESCE(commission_per_pallet, 1) INTO v_platform_fee
...
EXCEPTION
  WHEN OTHERS THEN
    RETURN jsonb_build_object('success', false, 'error', 'حدث خطأ غير متوقع: ' || SQLERRM);
END;
$$;
