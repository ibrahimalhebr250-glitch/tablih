...
import { ArrowRight, Plus, Package, MapPin, CreditCard as Edit3, Pause, Play, Trash2, RefreshCw, Wrench, DollarSign, Image as ImageIcon, X, ImagePlus, ChevronLeft, ChevronRight, Star, ShoppingCart, Warehouse } from 'lucide-react';
...
type InventoryTab = 'supplier' | 'purchased';
...
export default function SupplierInventory({ phone, onClose, onAddInventory }: Props) {
  const [activeTab, setActiveTab] = useState<InventoryTab>('supplier');
  ...
  const { items: purchasedItems, summary: purchasedSummary, loading: purchasedLoading } = useBuyerInventory(phone);
  const [selectedPurchasedItem, setSelectedPurchasedItem] = useState<any>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center">
      ...
      >
        ...
        {/* Tabs */}
        <div className="flex gap-2 px-4 pt-3 pb-2" style={{ background: '#e8f1f7' }} dir="rtl">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[13px] font-bold transition-all ${
              activeTab === 'supplier'
                ...
                : 'bg-transparent text-[#7a9aab]'
            }`}
          >
            <Package className="w-4 h-4" />
            <span>مخزون التوريد</span>
            {listings.length > 0 && (
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                activeTab === 'supplier' ? 'bg-[#27AE60] text-white' : 'bg-[#7a9aab]/20 text-[#7a9aab]'
              }`}>
                {listings.length}
              </span>
            )}
          ...
          >
            ...
            <span>مشترياتي</span>
            {purchasedSummary && purchasedSummary.total_items > 0 && (
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                activeTab === 'purchased' ? 'bg-[#10b981] text-white' : 'bg-[#7a9aab]/20 text-[#7a9aab]'
              }`}>
                {purchasedSummary.total_items}
              </span>
            )}
          </button>
        </div>
            ...
            {purchasedSummary && (
              <div className="grid grid-cols-2 gap-3 mb-4 mt-3">
                <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl p-3 border border-emerald-200">
                  ...
                  <p className="text-2xl font-bold text-emerald-900">{purchasedSummary.total_pallets.toLocaleString()}</p>
                  <p className="text-xs text-emerald-600 mt-1">من {purchasedSummary.total_items} صفقة</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-3 border border-blue-200">
                  ...
                  <p className="text-2xl font-bold text-blue-900">{purchasedSummary.total_value.toLocaleString()}</p>
            ...
            {purchasedLoading ? (
              ...
              <div className="space-y-3">
                {purchasedItems.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => setSelectedPurchasedItem(item)}
        ...
        >
          ...
          >
            ...
            <div className="p-4 space-y-4 overflow-y-auto" style={{ maxHeight: 'calc(85vh - 70px)' }}>
              {selectedPurchasedItem.images && selectedPurchasedItem.images.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedPurchasedItem.images.map((img: string, idx: number) => (
                      <img
                        key={idx}
                        src={img}
                        alt={`صورة ${idx + 1}`}
              ...
              {selectedPurchasedItem.description && (
                <div>
              ...
              <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                <DetailRow label="النوع" value={selectedPurchasedItem.pallet_type} />
                <DetailRow label="الحجم" value={selectedPurchasedItem.size} />
                <DetailRow label="الدرجة" value={selectedPurchasedItem.quality} />
                {selectedPurchasedItem.condition && (
                  <DetailRow label="الحالة" value={selectedPurchasedItem.condition} />
                )}
                <DetailRow label="الكمية" value={`${selectedPurchasedItem.quantity} طبلية`} />
                <DetailRow label="المدينة" value={selectedPurchasedItem.city} />
                <DetailRow label="سعر الوحدة" value={`${selectedPurchasedItem.unit_price} ر.س`} />
                <DetailRow label="المبلغ الإجمالي" value={`${selectedPurchasedItem.total_paid} ر.س`} />
                <DetailRow label="المورد" value={selectedPurchasedItem.original_supplier_name} />
                <DetailRow label="رقم الصفقة" value={selectedPurchasedItem.deal_ref} />
                <DetailRow
                  label="تاريخ الشراء"
                  value={new Date(selectedPurchasedItem.acquired_at).toLocaleDateString('ar-SA', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                  })}
