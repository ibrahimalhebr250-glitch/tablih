/*
  # Fix create_invoice_for_deal - incorrect column references

  1. Problem
    - Function references `ib.pallet_size` but the actual column is `ib.size`
    - Function references `ib.quality_grade` but the actual column is `ib.quality`
    - This causes "column ib.pallet_size does not exist" error when supplier confirms delivery
      because the deal status changes to 'completed' which triggers invoice auto-generation

  2. Changes
    - Replace `ib.pallet_size` with `ib.size`
    - Replace `ib.quality_grade` with `ib.quality`
*/
  ...
  ELSIF p_invoice_type = 'supplier_invoice' THEN
    ...
    v_items := jsonb_build_array(
      jsonb_build_object(
        ...
        'quantity', v_deal.quantity,
        'unit_price', v_deal.platform_commission / GREATEST(v_deal.quantity, 1),
